...
| Component | Threads |
|---|---|
| Self-play | 1 thread, 1 evaluator, G games, K = 1 (§5.5). **M4a (planned):** T search threads + 1 evaluation thread, one evaluator, G the global concurrency; or N processes of the first version, each with its own G (§5.5.1). |
| Match | 1 thread, 2 evaluators, G game pairs. **M4a (planned):** T search threads + 1 evaluation thread owning both evaluators, requests bucketed by side (§5.5.1). |
