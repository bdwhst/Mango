...
**Multi-threaded self-play (M4a, DESIGN §5.5.1) from iteration 24.** Training was stopped on 2026-09-25 at 23:33 in iteration 23's self-play (best `0022`; 5 of the iteration's chunks published, plan single-process, `games_in_flight` 128). The run config was changed on 2026-09-25 after the benchmarks below to `selfplay.threads = 8`, `selfplay.games_in_flight = 1024`, `selfplay.processes = 1`, `eval.threads = 4`; iteration 23 finishes on its already persisted plan when the run is resumed (the driver reads `games_in_flight` and `threads` from the config at launch, so the remainder of iteration 23 already runs with them), and from iteration 24 every self-play phase is one `mango_selfplay` process with 8 search threads and 1,024 games in flight, the gate and ladder matches run with 4 search threads (the pipeline passes `--games-in-flight 1024`, i.e. every game of a 200-pair match at once). Per game the search is unchanged (DESIGN §5.5.1: every game equals its sequential run under a deterministic evaluator); with the fp16 network the batch shapes change the rounding, so games from iteration 24 on are not bit-reproducible from their seeds, and G = 1024 instead of 128 changes which games run together, not their distribution. Benchmarks on `0022`, 900 games per configuration, S = 200, fp16, wall time including start-up — step 0 (processes): 1×G128 299 positions/s, 3×G43 232, 2×G128 430, 3×G128 485, 4×G128 501; step 1 (threads, one process): T1 G128 293, T2 G128 324, T4 G128 311, T8 G128 291 (batches halve at a fixed G, evaluator-bound), T4 G256 543, T4 G512 595, T8 G512 690, **T8 G1024 820** (M4a acceptance ≥ 700 met). Gate, 200 pairs `0022` vs `0021`: T1 G128 189 s, T4 G128 251 s, **T4 G400 117 s**, T8 G400 128 s (acceptance ≤ 90 s not met; DESIGN §11 M4a).
